...
LEVEL_TIME_LIMIT = 5000  # Level duration in milliseconds (5 seconds)
...
# Function to send score to Discord
    ...
    while True:
        ...
        while True:
            ...
            if check_collision(player_rect, obstacles):
                ...
                display_text("Press 'R' to Retry or 'Q' to Quit", font, WHITE, (WIDTH // 3, HEIGHT // 2 + 50))
                pygame.display.flip()

                # Send the score to Discord when the game is over
            ...
            # Check if the player has survived the level (by using time)
            if pygame.time.get_ticks() - level_start_time > LEVEL_TIME_LIMIT:  # 5 seconds per level
